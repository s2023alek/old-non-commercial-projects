...
package test000 {
	 ...
	 */
	public class Flake extends SceneObject {
		 ...
		 */
		function Flake(size:uint, scrW:uint, scrH:uint, movementSpeed:uint, screenZ:uint) {
			super(
				null//35
				,Math.max(35, size*screenZ/100)
				,Math.max(35, size*screenZ/100)
			);
		...
		private function createBD(selectedMode:Boolean=false):void {
			...
			var generatedRndColor:uint=Math.random()*20+80+150*sZ/100;
			generatedRndColor=(generatedRndColor << 16 | generatedRndColor << 8 | generatedRndColor);
			//snowflake draw options
			var qw:uint=get_w()/8;
			var dw:uint=get_w()/12;
			var ax:uint=Math.random()*dw/3+dw/2;
			var ay:uint=Math.random()*qw*2
			
			var bx:uint=Math.random()*dw/2+dw/4
			var by:uint=Math.random()*qw*1.5+qw
			
			var cx:uint=Math.random()*dw/4+dw/6
			var cy:uint=Math.random()*qw+qw*2
			
			for (var i:uint = 0; i < 5; i++) {
				sh=shapes[i];
				
				sh.graphics.clear();
				sh.graphics.beginFill(generatedRndColor);
				drawSnowflakeSide(sh, ax, ay, bx, by, cx, cy, get_w()/2);
				sh.graphics.endFill();
			...
			//create bitmap(s)
			var defBD:BitmapData=new BitmapData(get_w(),get_h(),true,0);
			defBD.draw(sp);
			addFrame(defBD);
			nextFrame();
		}
		...
		/**
		 * not used due to severe performance fall
		 */
		private function drawSnowflakeSide(sp:Shape, aX:uint, aY:uint, bX:uint, bY:uint, cX:uint, cY:uint, w:uint):void {
			sp.graphics.moveTo(0, 0);
			sp.graphics.lineTo(aX, aY);
			sp.graphics.lineTo(bX, bY);
			sp.graphics.lineTo(cX, cY);
			sp.graphics.lineTo(0, w);
			sp.graphics.lineTo(-cX, cY);
			sp.graphics.lineTo(-bX, bY);
			sp.graphics.lineTo(-aX, aY);
